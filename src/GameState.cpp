...
{
	if (!this->font.loadFromFile("Resources/Fonts/BodoniModa-VariableFont_opsz,wght.ttf"))
	{
...
{
	if (!this->textures["PLAYER_SHEET"].loadFromFile("Resources/Images/Sprites/Player/cat2.png"))
	{
...
{
	this->tileMap = new TileMap("Resources/text.slmp");
}

void GameState::initSystems()
{
	this->tts = new TextTagSystem("Resources/Fonts/BodoniModa-VariableFont_opsz,wght.ttf");
}
...
{
	if (!this->player->isDead() && sf::Keyboard::isKeyPressed(sf::Keyboard::Key(this->keybinds.at("CLOSE"))) && this->getKeytime())
	{
		if (!this->paused)
			this->pauseState();
		else
...
void GameState::updateCombat(Enemy* enemy, int index, float dt)
{
...
void GameState::updateDebugText(float dt)
{
	std::stringstream ss;
...
void GameState::update(float dt)
{
	...
	if (this->player->isDead())
	{
		if (!this->paused)
			this->pauseState();
		this->pmenu->setMenuText("Game over!");
	}
}
